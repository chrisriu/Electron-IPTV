...
export class XtreamCodeAPIService {
    ...
    sendLoginRequest(username: string, password: string){
      const url = `${config.url}:${config.port}/player_api.php?username=${username}&password=${password}`
      return this.httpClient.get(url)
    }

    sendCategoryRequest(username: string, password: string, category_name: string){
      const url = `${config.url}:${config.port}/player_api.php?username=${username}&password=${password}&action=${category_name}`
      return this.httpClient.get<Category[]>(url)
    }

    sendLiveTVStreamRequest(username: string, password: string){
      const url = `${config.url}:${config.port}/player_api.php?username=${username}&password=${password}&action=get_live_streams`
      return this.httpClient.get<LiveTV[]>(url)
    }

    sendVodStreamRequest(username: string, password: string){
      const url = `${config.url}:${config.port}/player_api.php?username=${username}&password=${password}&action=get_vod_streams`
      return this.httpClient.get<Movie[]>(url)
    }
    
    sendSerieStreamRequest(username: string, password: string){
      const url = `${config.url}:${config.port}/player_api.php?username=${username}&password=${password}&action=get_series`
